...
type Asset = { id: string; workflow_stage: string; image_type: string; status: string };
...
export function CaseCtSegmentationEvidence({ apiBaseUrl, authorizedFetch, caseId, analysisId }: { apiBaseUrl: string; authorizedFetch: AuthorizedFetch; caseId: string; analysisId?: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) setAsset(selected);
