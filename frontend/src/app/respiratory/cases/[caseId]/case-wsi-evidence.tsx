...
export function CaseWsiEvidence({ apiBaseUrl, authorizedFetch, caseId, stain, fillHeight = false }: { apiBaseUrl: string; authorizedFetch: AuthorizedFetch; caseId: string; stain: "HE" | "PDL1"; fillHeight?: boolean }) {
  ...
  return <section className={`${fillHeight ? "flex h-full min-h-0 flex-col" : ""} overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm`}><header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 px-5 py-4"><div><p className="text-[10px] font-semibold text-blue-600">원본 근거 · WSI</p><h2 className="mt-1 text-base font-bold text-slate-900">{title}</h2><p className="mt-1 text-xs text-slate-500">검체별 슬라이드를 선택해 확대·축소하며 확인할 수 있습니다.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{slides.length}개</span></header><div className={`${fillHeight ? "min-h-0 flex-1" : "min-h-[380px]"} grid grid-cols-[180px_minmax(0,1fr)]`}><aside className="min-h-0 overflow-y-auto border-r border-slate-200 bg-slate-50/60 p-3"><p className="px-1 pb-2 text-[10px] font-bold text-slate-500">{stainName(stain)} 슬라이드</p>{loading ? <p className="p-3 text-xs text-slate-400">목록을 불러오는 중입니다.</p> : slides.length === 0 ? <p className="p-3 text-xs leading-5 text-slate-400">조회 가능한 슬라이드가 없습니다.</p> : <div className="space-y-2">{slides.map((slide) => { const specimen = specimens.find((item) => item.id === slide.specimen_id); const selected = slide.id === selectedSlideId; return <button key={slide.id} type="button" onClick={() => setSelectedSlideId(slide.id)} className={`w-full rounded-lg border p-3 text-left ${selected ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white hover:border-slate-300"}`}><div className="flex items-center justify-between gap-2"><span className="truncate text-xs font-bold text-slate-800">{slide.slide_code}</span><span className="text-[9px] font-bold text-violet-700">{stainName(stain)}</span></div><p className="mt-1 truncate text-[10px] text-slate-500">검체 {specimen?.specimen_code ?? "-"}</p><p className="mt-1 text-[10px] text-slate-400">{slide.mpp ?? "-"} μm/px</p></button>; })}</div>}</aside><div className="flex min-h-0 min-w-0 flex-col"><div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-4 py-2.5"><div className="min-w-0"><p className="truncate text-xs font-bold text-slate-800">{selectedSlide?.slide_code ?? "슬라이드를 선택하세요"}</p><p className="truncate text-[10px] text-slate-400">{selectedSpecimen ? `${selectedSpecimen.specimen_code}${selectedSpecimen.body_site ? ` · ${selectedSpecimen.body_site}` : ""}` : ""}</p></div><div className="flex shrink-0 gap-1"><Tool label="축소" onClick={() => zoom(0.8)} disabled={!viewerData} /><Tool label="확대" onClick={() => zoom(1.25)} disabled={!viewerData} /><Tool label="초기화" onClick={() => viewerRef.current?.viewport.goHome()} disabled={!viewerData} /><Tool label="전체화면" onClick={() => { void containerRef.current?.requestFullscreen?.(); }} disabled={!viewerData} /></div></div><div className={`${fillHeight ? "min-h-0 flex-1" : "min-h-[322px]"} relative bg-slate-950`}>{viewerData && selectedSlide && <div ref={containerRef} className="absolute inset-0" aria-label={`${selectedSlide.slide_code} WSI 뷰어`} />}{(loading || viewerLoading) && <p role="status" className={`${fillHeight ? "absolute inset-0" : "min-h-[322px]"} grid place-items-center text-sm text-slate-300`}>WSI를 불러오는 중입니다.</p>}{!loading && error && <p role="alert" className={`${fillHeight ? "absolute inset-0" : "min-h-[322px]"} grid place-items-center px-8 text-center text-sm text-rose-200`}>{error}</p>}{!loading && !error && !selectedSlide && <p className={`${fillHeight ? "absolute inset-0" : "min-h-[322px]"} grid place-items-center px-8 text-center text-sm text-slate-300`}>표시할 {stainName(stain)} WSI가 없습니다.</p>}{viewerData && selectedSlide && <span className="pointer-events-none absolute bottom-3 left-3 rounded bg-slate-950/80 px-2 py-1 text-[10px] text-white">휠로 확대·축소 · 드래그로 이동</span>}</div></div></div></section>;
}
