...
it.each(["CASE_CLOSED", "REFERRED_OUT"])("confirms CT before %s using existing contracts", async (action) => {
  const authorizedFetch = vi.fn()
    .mockResolvedValueOnce(response({ id: "ct-1" }))
    .mockResolvedValueOnce(response({ id: "ct-1", result_status: "CONFIRMED" }))
    .mockResolvedValueOnce(response({ case_status: action === "CASE_CLOSED" ? "CLOSED" : "REFERRED_OUT" }));
  const onCompleted = vi.fn();
  render(<CtWorkflowDecision caseId="case-1" aiResultId="ai-1" authorizedFetch={authorizedFetch} onCompleted={onCompleted} />);
  fireEvent.click(screen.getByRole("button", { name: "결과 입력 및 처리" }));
  fireEvent.change(screen.getByLabelText("종합 판정"), { target: { value: "NO_NODULE" } });
  expect(screen.getByLabelText("처리 방법")).toHaveValue("CASE_CLOSED");
  ...
  const submit = screen.getByRole("button", { name: action === "CASE_CLOSED" ? "결과 확정 및 Case 종료" : "결과 확정 및 의뢰 처리" });
  ...
  expect(JSON.parse(authorizedFetch.mock.calls[2][1].body)).toMatchObject({ action, source_clinical_result_id: "ct-1", target_stage: null });
});
...
it("retries only the downstream decision after CT confirmation succeeded", async () => {
  const authorizedFetch = vi.fn()
    ...
    .mockResolvedValueOnce(response({ detail: "종료 실패" }, 400))
    .mockResolvedValueOnce(response({ case_status: "CLOSED" }));
  const onCompleted = vi.fn();
  render(<CtWorkflowDecision caseId="case-1" aiResultId="ai-1" authorizedFetch={authorizedFetch} onCompleted={onCompleted} />);
  fireEvent.click(screen.getByRole("button", { name: "결과 입력 및 처리" }));
  fireEvent.change(screen.getByLabelText("종합 판정"), { target: { value: "NO_NODULE" } });
  fireEvent.change(screen.getByPlaceholderText("결정 사유"), { target: { value: "정상" } });
  fireEvent.click(screen.getByRole("button", { name: "결과 확정 및 Case 종료" }));
  expect(await screen.findByRole("alert")).toHaveTextContent("종료 실패");
  expect(screen.getByLabelText("종합 판정")).toBeDisabled();
  expect(screen.getByPlaceholderText("결정 사유")).toHaveValue("정상");
...
it("loads a confirmed CT result delivered after mount and skips save/confirm", async () => {
  const authorizedFetch = vi.fn().mockResolvedValue(response({ case_status: "CLOSED" }));
