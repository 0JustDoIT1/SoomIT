...
export function TnmReviewWorkspace({ aiTnm, clinicalTnm, modelName, modelVersion, caseId, apiBaseUrl, authorizedFetch, onDirtyChange }: { aiTnm?: AiTnm; clinicalTnm?: ClinicalTnm; modelName?: string; modelVersion?: string; caseId?: string; apiBaseUrl?: string; authorizedFetch?: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>; onConfirmed?: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const confirmDraft = async () => { if (!authorizedFetch || !apiBaseUrl || !caseId || !savedId) return; const r = await authorizedFetch(`${apiBaseUrl}/api/doctor/cases/${caseId}/clinical-results/tnm/${savedId}/confirm/`, { method: "POST" }); setMessage(r.ok ? "TNM ?? ??" : "TNM ?? ??"); if (r.ok) onConfirmed?.() };
  const selectCategory = (next: TnmCategory) => { setCategory(next); requestAnimationFrame(() => tabRefs.current[next]?.focus()); };
  const calculateStage = async () => { if (!authorizedFetch || !apiBaseUrl || !caseId || !savedId) return; const r = await authorizedFetch(`${apiBaseUrl}/api/doctor/cases/${caseId}/clinical-results/tnm/${savedId}/stage/`, { method: "POST" }); setMessage(r.ok ? "Stage ?? ??" : "Stage ?? ??"); if (r.ok) onConfirmed?.(); }
  const confirmStage = async () => { if (!authorizedFetch || !apiBaseUrl || !caseId || !savedId) return; const r = await authorizedFetch(`${apiBaseUrl}/api/doctor/cases/${caseId}/clinical-results/tnm/${savedId}/stage/confirm/`, { method: "POST" }); setMessage(r.ok ? "Stage Group ?? ??" : "Stage Group ?? ??"); if (r.ok) onConfirmed?.(); }
  ...
  return (
    <section className="h-full min-h-0 overflow-x-auto overflow-y-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <div className="flex items-center gap-2 border-t border-slate-200 px-3 py-2"><button type="button" onClick={saveDraft} className="rounded bg-blue-600 px-3 py-1 text-xs font-semibold text-white">Draft ??</button><button type="button" onClick={confirmDraft} disabled={!savedId} className="rounded bg-emerald-600 px-3 py-1 text-xs font-semibold text-white disabled:bg-slate-200">?? ??</button><button type="button" onClick={calculateStage} disabled={!savedId} className="rounded bg-violet-600 px-3 py-1 text-xs font-semibold text-white disabled:bg-slate-200">Stage ??</button><button type="button" onClick={calculateStage} disabled={!savedId} className="rounded bg-violet-600 px-3 py-1 text-xs font-semibold text-white disabled:bg-slate-200">Stage ??</button><button type="button" onClick={confirmStage} disabled={!savedId || !clinicalTnm?.evidence?.stage?.stage_group_candidate || clinicalTnm?.stage_group != null && clinicalTnm.stage_group !== ""} className="rounded bg-emerald-700 px-3 py-1 text-xs font-semibold text-white disabled:bg-slate-200">Stage Group ?? ??</button>{message && <span className="text-xs text-slate-600">{message}</span>}</div>
