...
type Source = { document: string; chunk_index: number; distance: number; excerpt?: string };
...
export function KnowledgeRagPanel({ apiBaseUrl, authorizedFetch }: { apiBaseUrl: string; authorizedFetch: AuthorizedFetch }) {
  ...
  return <section className="rounded-lg border border-slate-200 bg-white p-4">
    ...
    <p className="mt-1 text-xs text-slate-600">실제 지식 문서 검색 결과만 근거로 답변합니다. AI 답변은 의료진 판단을 대체하지 않습니다.</p>
    <div className="mt-3 flex gap-2"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void ask(); }} maxLength={1000} placeholder="예: EGFR 변이 치료 근거를 요약해줘" className="min-w-0 flex-1 rounded-md border border-slate-300 px-3 py-2 text-xs" /><button type="button" disabled={!question.trim() || loading} onClick={() => void ask()} className="rounded-md bg-blue-600 px-3 py-2 text-xs font-semibold text-white disabled:bg-slate-300">{loading ? `질의 중 ${elapsedSeconds}초` : "질의"}</button>{loading && <button type="button" onClick={() => controllerRef.current?.abort()} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-600">취소</button>}</div>
    ...
    {answer && <div className="mt-3 rounded-md border border-blue-100 bg-blue-50/40 p-3"><p className="whitespace-pre-wrap text-xs leading-5 text-slate-700">{answer}</p>{sources.length > 0 && <ul aria-label="답변 근거 문서" className="mt-3 border-t border-blue-100 pt-2 text-[10px] text-slate-500">{sources.map((source, sourceOrder) => <li key={`${source.document}-${source.chunk_index}`} className="py-1"><details><summary className="cursor-pointer">[{sourceOrder + 1}] {source.document} <span className="text-slate-400">(문서 조각 {source.chunk_index + 1})</span></summary>{source.excerpt && <p className="mt-1 whitespace-pre-wrap rounded bg-white px-2 py-1 text-slate-600">{source.excerpt}</p>}</details></li>)}</ul>}</div>}
  </section>;
}
