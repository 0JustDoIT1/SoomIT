...
export function PrescriptionPanel({ caseId, apiBaseUrl, authorizedFetch, refreshKey = 0, actionable = true, waitingMessage, hasSelectedRegimen = false, onPrescriptionChanged }: Props) {
  ...
  const creationForm = (
      <details className="shrink-0 border-t border-slate-200 pt-2" open={prescriptions.length === 0}>
        ...
        <div className="mt-2 flex flex-wrap items-end gap-2">
          ...
          <input type="date" value={cycleStartDate} onChange={e => setCycleStartDate(e.target.value)} className="rounded-lg border border-slate-200 p-2 text-xs" aria-label="Cycle 시작일" />
          <button type="button" disabled={working || !hasSelectedRegimen} onClick={() => void create()} className={`rounded-lg px-4 py-2 text-sm font-semibold disabled:bg-slate-200 disabled:text-slate-500 ${prescriptions.length ? "border border-slate-300 bg-white text-slate-700" : "bg-blue-600 text-white"}`}>{working ? "생성 중..." : "임시 처방 생성"}</button>
  ...
  return <section className="flex h-full min-h-0 flex-col gap-2 rounded-lg border border-slate-200 bg-white p-3" aria-label="처방 작업공간">
    ...
    {!actionable ? <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">{waitingMessage ?? "치료계획 최종 확정 후 처방 작성 가능"}</p> : <>
      ...
      {active ? <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)] gap-3" key={active.id}>
        <section className="min-h-0 overflow-y-auto pr-1" aria-label="처방 약물 목록">
          ...
          {active.items.map(item => <ItemRow key={item.id} item={item} editable={active.prescription_status === "DRAFT"} working={working} apiBaseUrl={apiBaseUrl} authorizedFetch={authorizedFetch} onSave={body => updateItem(active.id, item.id, body)} />)}
        ...
        <aside className="flex min-h-0 flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3" aria-label="안전성 검토 및 최종 확정">
          ...
          {active.safety_check_results.some(r => r.result === "WARNING" && !r.acknowledged_at) && <p role="alert" className="shrink-0 rounded-lg border border-amber-300 bg-amber-50 p-2 text-xs font-semibold text-amber-800">WARNING · 의료진 검토와 확인 사유가 필요합니다.</p>}
          ...
          {active.prescription_status === "DRAFT" && <button type="button" disabled={working} onClick={() => void safety(active.id)} className="shrink-0 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{working ? "실행 중..." : "안전성 검사 실행"}</button>}
          {active.prescription_status === "VALIDATED" && active.safety_check_results.some(r => r.result === "WARNING" && !r.acknowledged_at) && <button type="button" disabled={working} onClick={() => void acknowledge(active.id)} className="shrink-0 rounded-lg bg-amber-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">WARNING 확인</button>}
          {active.prescription_status === "VALIDATED" && !active.safety_check_results.some(r => r.result === "BLOCK" || (r.result === "WARNING" && !r.acknowledged_at)) && <div className="flex min-h-0 flex-1 flex-col"><PrescriptionFinalizeScheduleForm items={active.items} working={working} onFinalize={async schedules => finalize(active.id, schedules)} /></div>}
      ...
      </div> : <p className="min-h-0 flex-1 rounded-lg bg-slate-50 p-3 text-sm text-slate-500">등록된 처방이 없습니다.</p>}
...
function ItemRow({ item, editable, working, apiBaseUrl, authorizedFetch, onSave }: { item: Item; editable: boolean; working: boolean; apiBaseUrl: string; authorizedFetch: AuthorizedFetch; onSave: (body: object) => Promise<boolean> }) {
  ...
  return <div className="mt-3 rounded border border-slate-100 bg-white p-3"><div className="grid gap-2 text-xs sm:grid-cols-2"><p className="font-semibold text-slate-700">{item.drug_name}<span className="ml-1 font-normal text-slate-400">{item.ingredient_name}</span></p><p className="text-slate-500">계산 {item.calculated_dose ?? "-"}{item.unit ?? ""} · 최종 {item.final_dose ?? "-"}{item.unit ?? ""} · {item.route}</p></div>{item.instructions && <p className="mt-1 text-xs text-slate-600">투여 지시 · {item.instructions}</p>}<MfdsProductSelector ingredientName={item.ingredient_name ?? item.drug_name} selectedItemSeq={item.mfds_item_seq} apiBaseUrl={apiBaseUrl} authorizedFetch={authorizedFetch} onSelect={async (mfdsItemSeq) => { await onSave({ mfds_item_seq: mfdsItemSeq }); }} />{editable && <div className="mt-3 grid gap-2 grid-cols-[90px_minmax(0,1fr)] [&>button]:col-span-2"><input type="number" min="0" value={finalDose} onChange={(e) => setFinalDose(e.target.value)} className="rounded border border-slate-200 px-3 py-2 text-sm" aria-label={`${item.drug_name} 최종 용량`} /><input value={instructions} onChange={(e) => setInstructions(e.target.value)} className="rounded border border-slate-200 px-3 py-2 text-sm" aria-label={`${item.drug_name} 투여 지시`} /><button type="button" disabled={working || finalDose === ""} onClick={() => void onSave({ final_dose: finalDose, instructions })} className="rounded border border-emerald-200 px-3 py-2 text-sm font-semibold text-emerald-700 disabled:opacity-50">수정 저장</button></div>}</div>;
}
