...
export function OrderManagementItem({ caseId, order, authorizedFetch, onChanged }: { caseId: string; order: ExaminationOrder; authorizedFetch: AuthorizedFetch; onChanged: () => void }) {
  ...
  return <article className="rounded-md border border-slate-100 bg-slate-50 p-3 text-xs">
    <div className="grid grid-cols-2 gap-x-3 gap-y-1"><Field label="검사" value={order.order_type_label} /><Field label="상태" value={statusLabel(order.status)} /><Field label="우선순위" value={order.priority === "URGENT" ? "긴급" : "일반"} /><Field label="생성 시각" value={formatDateTime(order.created_at)} /><Field label="예약 일시" value={order.scheduled_at ? formatDateTime(order.scheduled_at) : "미배정"} /><div className="col-span-2"><Field label="검사 목적" value={order.purpose || "-"} /></div></div>
