...
export function CasePatientSidebar({ cases, selectedId, searchText, onSearchChange, onSelect }: Props) {
  ...
  return (
    <aside className="flex h-full min-h-0 w-full flex-col overflow-hidden border-r border-slate-200 bg-white shadow-[2px_0_12px_rgba(15,23,42,0.03)]">
      ...
      <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto bg-slate-50/60 p-1.5 [scrollbar-gutter:stable]">
        {visibleCases.map((item) => (
          <button key={item.id} type="button" onClick={() => onSelect(item.id)} aria-current={item.id === selectedId ? "true" : undefined} className={`w-full rounded-lg border px-2.5 py-2.5 text-left transition ${item.id === selectedId ? "border-blue-300 bg-blue-50 shadow-[inset_3px_0_0_#2563eb,0_4px_12px_rgba(37,99,235,0.08)]" : "border-slate-200 bg-white hover:border-blue-200 hover:bg-white hover:shadow-sm"}`}>
            ...
            <p className="mt-1.5 truncate text-[11px] font-medium text-slate-600">{item.patient_code || "-"}</p>
