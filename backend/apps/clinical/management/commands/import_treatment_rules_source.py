"""Validate and atomically import the four prototype treatment rules."""
import csv
import json
...
OMITTED = ("histology", "stage_condition", "pdl1_condition", "ecog_condition", "treatment_line")


class Command(BaseCommand):
    help = "Import prototype TR01/TR04/TR05 CSV; use --dry-run to validate without writes."
    ...
    def read_rows(self, path):
        ...
        for line, row in enumerate(rows, 2):
            ...
            if (row["cancer_type"] != "NSCLC"
                    or row["biomarker_condition"] != ALTERATIONS[row["rule_code"]]
                    or any(row[field] is not None for field in OMITTED)
                    or not row["evidence_source"]):
                raise CommandError(f"Line {line}: conditions/evidence differ from the prototype contract.")
        if keys != set(EXPECTED):
            raise CommandError("CSV must contain exactly TR01/R1, TR01/R2, TR04/R5, TR05/R6.")
        return rows
